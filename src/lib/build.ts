...
// Track generated global modules for injection
type GlobalModules = {
  runtime: string[]  // paths to inject at runtime
  types: string[]    // module names for .d.ts generation
}

/**
 * Import a module with alias resolution and global injection using esbuild.
 * Falls back to direct import when no aliases or globals are configured.
 */
async function importWithAliases(
  filePath: string,
  aliases: AliasConfig | undefined,
  sourcePath: string,
  injectGlobals?: string // path to _globals.mjs
): Promise<any> {
  // If no aliases and no globals configured, use direct import with cache-busting
  if ((!aliases || Object.keys(aliases).length === 0) && !injectGlobals) {
  ...
  // Use esbuild to bundle with alias resolution and global injection
  const result = await esbuild.build({
    ...
    // Inject globals ($theme, $device, etc.) so they're available without import
    inject: injectGlobals ? [injectGlobals] : [],
...
export async function build(
  ...
  // 3.5. Generate _globals.mjs for auto-injection and globals.d.ts for TypeScript
  const globalModuleNames: string[] = []
  if (config.media) {
    for (const mediaType of Object.keys(config.media)) {
      globalModuleNames.push(mediaType) // 'theme', 'device', etc.
    }
  }

  let globalsInjectPath: string | undefined
  if (globalModuleNames.length > 0) {
    // Generate _globals.mjs (runtime injection)
    const globalsRuntime = globalModuleNames
      .map((name) => `export { default as $${name} } from './$${name}.mjs'`)
      .join('\n')
    const globalsRuntimePath = path.join(sourcePath, '_globals.mjs')
    await fs.writeFile(globalsRuntimePath, globalsRuntime + '\n', 'utf8')
    globalsInjectPath = globalsRuntimePath

    // Generate globals.d.ts (TypeScript support)
    const globalsTypes = `// Auto-generated by esm-styles - provides global $theme, $device, etc.
// Add to tsconfig.json: "include": ["path/to/source/globals.d.ts"]

${globalModuleNames
  .map(
    (name) => `declare const $${name}: typeof import('./$${name}.mjs').default`
  )
  .join('\n')}
`
    const globalsTypesPath = path.join(sourcePath, 'globals.d.ts')
    await fs.writeFile(globalsTypesPath, globalsTypes, 'utf8')
  }
  ...
  for (const floor of floors) {
    ...
    const stylesObj = (
      await importWithAliases(inputFile, aliases, sourcePath, globalsInjectPath)
    ).default
